...
export default function DrawChart() {
  ...
  useEffect(() => {
    getPalette()
      ...
      .catch(() => {
        Taro.showToast({ title: '色卡加载失败', icon: 'none' })
      })
  }, [])
  ...
  const stats = useMemo(() => {
    ...
    return { total, colorCount: Object.keys(map).length }
  }, [grid])
              ...
              fail: (e: any) => {
                if (e.errMsg && e.errMsg.includes('auth deny')) {
                  Taro.showModal({
                    ...
                    confirmText: '去设置',
                    success: (rr) => rr.confirm && Taro.openSetting(),
                  })
                } else {
  ...
  return (
    <View className='page'>
      ...
      <ScrollView scrollX className='tool-bar'>
        ...
        <View
          className={`tool-btn ${history.length === 0 ? 'disabled' : ''}`}
          onClick={undo}
        >
          ↶ 撤销
        </View>
        <View
          className={`tool-btn ${future.length === 0 ? 'disabled' : ''}`}
          onClick={redo}
        >
          ↷ 重做
        </View>
        <View className='tool-btn' onClick={clearAll}>
          🗑 清空
        </View>
        <View className='tool-btn primary' onClick={saveToAlbum}>
          💾 保存
        </View>
      </ScrollView>
            ...
            <View className='canvas-row' key={y}>
              {row.map((code, x) => (
                <View
                  ...
                  style={{
                    backgroundColor: code ? getHex(code) : '#ffffff',
                  }}
        ...
        {(paletteGroups[activeSeries] || []).map((c) => (
          <View
            ...
            className={`color-cell ${
              selectedColor?.code === c.code ? 'active' : ''
            }`}
